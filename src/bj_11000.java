...
public class bj_11000 {
    ...
    static class Lesson implements Comparable<Lesson> {
        ...
        public int compareTo(Lesson o) {
            if(this.start == o.start){
                return this.end - o.end; //종료시간도 오름차순(빨리 끝날수록 강의실을 더 많이 배정가능)
            }
            return this.start - o.start; // 시작시간은 오름차순 (빠를수록 강의실을 빨리 배정받는다)
        }
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        int n = Integer.parseInt(st.nextToken()); //강의수
        for(int i=0; i<n; i++){
        ...
        for(Lesson le: lessons) {
            ...
            if(!pq.isEmpty() && (pq.peek() <= le.start)){
                //현재 강의 종료시간 <= 새로운 강의 시작시간 ===> 같은 강의실을 배정(현재 강의은 새로운 강의로 바뀜)
                pq.poll();
            }
            //현재 강의 종료 시간 > 새로운 강의 시작시간 ===> 다른 강의실을 배정
            pq.add(end);
        }
